...
from dashboard.server import start as start_dashboard, push_state
...
def build_dashboard_state(risk: RiskManager, tracker: Tracker,
                           regimes: dict, positions: dict) -> dict:
    """Assemble the full state snapshot for the dashboard."""
    open_pos = []
    for strat_positions in positions.values():
        for pair, pos in strat_positions.items():
            p = {
                "pair":        pos.pair,
                "strategy":    getattr(pos, "strategy", "unknown"),
                "entry_price": pos.entry_price,
                "stop_loss":   pos.stop_loss,
                "take_profit": getattr(pos, "take_profit", getattr(pos, "target", None)),
    ...
    return {
        ...
        "portfolio":       round(risk.portfolio, 4),
        "available":       round(risk.available, 4),
        "open_positions":  risk.open_positions,
        "max_positions":   config.MAX_POSITIONS,
        "daily_pnl":       round(risk.daily_pnl, 4),
        "halted":          risk.halted,
        "total_pnl":       round(tracker.total_pnl, 4),
        "total_trades":    tracker.total_trades,
        "wins":            tracker.wins,
        "losses":          tracker.losses,
        "win_rate":        round(tracker.win_rate, 2),
        "profit_factor":   round(tracker.profit_factor, 3) if tracker.profit_factor != float("inf") else None,
        "by_strategy":     tracker.by_strategy(),
        "recent_trades":   tracker.trades[-50:],
        "regimes":         regimes,
        "positions":       open_pos,
    }
...
def main():
    ...
    while True:
                    ...
                    if regime == RANGING:
                        if grid_strat.should_enter(pair, price, risk.available):
                            ...
                            if alloc >= config.MIN_ORDER_USD:
                                grid_strat.open_grid(pair, price, alloc)
                                risk.on_open(alloc)
        ...
        all_positions = {
            "momentum":      {p: pos for p, pos in mom_strat.positions.items()},
            "mean_reversion": {p: pos for p, pos in mr_strat.positions.items()},
        }
        # Tag positions with strategy name for dashboard display
        for strat_name, pos_dict in all_positions.items():
            for pos in pos_dict.values():
                pos.strategy = strat_name

        state = build_dashboard_state(risk, tracker, regimes, all_positions)
        push_state(state)
